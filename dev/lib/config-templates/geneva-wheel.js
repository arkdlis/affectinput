  ...
  sketch(p) {
    ...
    class CircleButton {
      ...
      draw(p) {
        ...
        //labels
        if (this.value === 4) {
          drawLabels(this.label, this.x, this.y, 12);
        }
      }
    }
    ...
    p.draw = function() {
      ...
      let diameter = canvasWidth*80/500
      if (mouseInCircle(p.mouseX, p.mouseY, canvasWidth/2, canvasWidth/2, diameter/2 + 2)) {
        p.cursor(p.HAND);
        isHovered = true;
        lastHoveredValue = {
      ...
      p.fill(p.color('#ddd'));
      p.arc(canvasWidth/2, canvasWidth/2+2, diameter, diameter, 0, p.PI);
      ...
      p.fill(p.color('#ddd'));
      p.arc(canvasWidth/2, canvasWidth/2-2, diameter, diameter, p.PI, 2*p.PI);
