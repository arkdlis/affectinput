...
const trial = new lab.flow.Sequence({
  content: [
    ...
    new lab.html.Screen({
      content: '<div id="emotion-input" class="six-emotions center labjs-absolute"></div>',
      messageHandlers: {
        'run': function() {
          // initialize widget
          let sixEmotions = new SixEmotions();
          sixEmotions.setAssetsDirectory('lib/assets/');
          sixEmotions.onChange((result) => {
          ...
          sixEmotions.init(window.document.getElementById('emotion-input'));
        },
        'end': () => {
